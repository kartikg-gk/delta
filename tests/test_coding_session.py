...
from delta_harness.session.replay import ProjectedState, project_records, find_tip
...
def test_project_records_with_prune() -> None:
    ...
    state = project_records(records)
    # r1, r2 pruned → r3 survives, prune1 summary inserted, then r4
    assert len(state.transcript) == 3
    assert isinstance(state.transcript[0], HumanEntry)  # r3
    assert isinstance(state.transcript[1], PruneSummaryEntry)  # prune1
    assert state.transcript[1].summary == "Discussed greetings"
    assert state.record_ids == ["r3", "prune1", "r4"]
